...
def sma_single(sec_id, start_date, end_date, sma_short, sma_long):

    yahooapi = DataAPI(which_API="yahoo")
    df = yahooapi.yahoo_pricing_data(sec_id=sec_id, start_date = start_date, end_date = end_date)

    bt = Backtesting(df)
    df = bt.SMA_single_parameter(sma_short = sma_short, sma_long = sma_long)
    ...
    fig.add_trace(
        go.Scatter(
            x=df['date'], y=df['sma_short'], mode = 'lines', name="SMA_Short", marker_color='rgba(45, 51, 235, 1)'
        )
    )

    fig.add_trace(
        go.Scatter(
            x=df['date'], y=df['sma_long'], mode = 'lines', name="SMA_Long", marker_color='rgba(199, 44, 219, 1)'
        )
    )
